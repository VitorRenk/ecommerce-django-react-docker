...
import productAPI from '../../mocks/product';  // Importa o mock
...
export const fetchProductList = (keyword ,pageNumber='') => async (dispatch) => {
  try {
    ...
    dispatch(productListFailure(error.message || error));
  }
};
...
export const fetchProductDetails = (id) => async (dispatch) => {
  try {
    ...
    dispatch(productDetailsFailure(error.message || error));
  }
};
...
export const createReview = (productId, review) => async (dispatch) => {
  try {
    ...
    dispatch(createReviewFailure(error.message || error));
  }
};
...
export const fetchTopRatedProducts = () => async (dispatch) => {
  try {
    ...
    dispatch(productTopFailure(error.message || error));
  }
};
